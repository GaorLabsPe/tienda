...
const happyEntrepreneurImg = '/src/assets/images/happy_entrepreneur_receipts_1780589650226.png';
...
const INITIAL_PRODUCTS: Product[] = [
  // Minimarket / Bodega
  { id: 'm1', name: 'Arroz Extra Costeño 5kg', price: 18.50, category: 'minimarket', stock: 24, minStock: 8, emoji: '🌾' },
  { id: 'm2', name: 'Leche Evaporada Gloria 400g', price: 4.20, category: 'minimarket', stock: 125, minStock: 15, emoji: '🥛' },
  { id: 'm3', name: 'Aceite Primor Premium 1L', price: 8.90, category: 'minimarket', stock: 4, minStock: 10, emoji: '🧴' }, // low stock
  { id: 'm4', name: 'Gaseosa Inca Kola 3L', price: 11.50, category: 'minimarket', stock: 15, minStock: 5, emoji: '🥤' },
  { id: 'm5', name: 'Azúcar Rubia Cartavio 1kg', price: 3.80, category: 'minimarket', stock: 78, minStock: 12, emoji: '🍬' },
  { id: 'm6', name: 'Detergente Opal Multiusos 1kg', price: 9.80, category: 'minimarket', stock: 18, minStock: 5, emoji: '🧼' },

  // Ferreteria
  { id: 'h1', name: 'Martillo Tramontina Acero 16oz', price: 24.50, category: 'hardware', stock: 6, minStock: 5, emoji: '🔨' },
  { id: 'h2', name: 'Taladro Percutor Bosch 550W', price: 189.00, category: 'hardware', stock: 1, minStock: 3, emoji: '🔌' }, // low stock
  { id: 'h2b', name: 'Caja de Herramientas Premium Stanley', price: 95.00, category: 'hardware', stock: 14, minStock: 4, emoji: '🧰' },
  { id: 'h3', name: 'Cinta Aislante 3M Negra 20m', price: 3.50, category: 'hardware', stock: 50, minStock: 10, emoji: '🏷️' },
  { id: 'h4', name: 'Kit de Destornilladores Stanley x6', price: 35.00, category: 'hardware', stock: 12, minStock: 4, emoji: '🪛' },
  { id: 'h5', name: 'Clavos de Acero 2 pulgadas x1kg', price: 12.00, category: 'hardware', stock: 15, minStock: 5, emoji: '🔩' },

  // Tienda de Ropa
  { id: 'c1', name: 'Polo Algodón Premium Pima', price: 45.00, category: 'clothing', stock: 30, minStock: 5, emoji: '👕' },
  { id: 'c2', name: 'Pantalón Jean Denim Stretch Clásico', price: 89.00, category: 'clothing', stock: 3, minStock: 8, emoji: '👖' }, // low stock
  { id: 'c3', name: 'Casaca Cortaviento Impermeable', price: 149.00, category: 'clothing', stock: 8, minStock: 4, emoji: '🧥' },
  { id: 'c4', name: 'Zapatillas Urbanas Blancas Unisex', price: 120.00, category: 'clothing', stock: 10, minStock: 3, emoji: '👟' },

  // Restaurante / Café
  { id: 'r1', name: 'Pollo a la Brasa - 1/4 Pecho Súper', price: 22.00, category: 'restaurant', stock: 45, minStock: 10, emoji: '🍗' },
  { id: 'r2', name: 'Lomo Saltado con Gajos de Papa', price: 32.00, category: 'restaurant', stock: 15, minStock: 3, emoji: '🥩' },
  { id: 'r3', name: 'Arroz con Pollo Criollo Norteño', price: 18.00, category: 'restaurant', stock: 2, minStock: 5, emoji: '🍛' }, // low stock
  { id: 'r4', name: 'Chicha Morada Natural Jarra 1.5L', price: 12.00, category: 'restaurant', stock: 30, minStock: 8, emoji: '🍷' },
  { id: 'r5', name: 'Ceviche de Pescado Clásico Tres Mares', price: 35.05, category: 'restaurant', stock: 20, minStock: 4, emoji: '🐟' }
];
...
export default function POSSimulator() {
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<'EFECTIVO' | 'YAPE' | 'PLIN' | 'VISA'>('EFECTIVO');
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(p => {
      const matchIndustry = selectedIndustry === 'all' || p.category === selectedIndustry;
    ...
    setCart(prev => {
      return prev.map((item, idx) => {
        if (idx === selectedCartIndex) {
          ...
          if (numpadMode === 'qty') {
            const safeQty = Math.max(0, Math.min(item.product.stock, parsedVal));
  ...
  const tryApplyCoupon = (code: string) => {
    ...
    if (formatted === 'GAOR30' || formatted === 'ODOO' || formatted === 'CAMPANA30') {
      setAppliedDiscountRate(0.30);
  ...
  return (
    <div className="bg-[#f0f2f5] text-slate-800 rounded-2xl overflow-hidden border border-slate-350 shadow-2xl relative font-sans select-none">
      ...
      <div className="bg-[#201d24] text-slate-200 px-4 py-2.5 flex items-center justify-between border-b border-[#16141a]">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center text-xs">
            <span className="font-extrabold text-[#9e7694] tracking-wider uppercase">Odoo<span className="text-white font-medium lowercase">19</span></span>
            <span className="mx-2 text-slate-500 font-bold">/</span>
            <span className="text-slate-300 font-semibold truncate max-w-[140px] sm:max-w-none">Punto de Venta (POS-01)</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 text-[11px] text-slate-400 font-medium">
          ...
          <span className="flex items-center gap-1.5">
            ...
            <span className="hidden md:inline text-slate-300">SUNAT Conectada</span>
          </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 h-auto lg:h-[620px] border border-slate-300 rounded-b-2xl overflow-hidden shadow-2xl">
        
        {/* ==================== LEFT AREA: Odoo Order Lines & Dynamic Numpad (45%) ==================== */}
        <div className="lg:col-span-5 flex flex-col bg-white border-r border-slate-300 h-[520px] lg:h-full">
          ...
          <div className="flex-1 overflow-y-auto p-3.5 bg-slate-50/55 space-y-1 scrollbar-thin">
            {cart.map((item, idx) => {
              ...
              const hasLineDiscount = item.lineDiscount !== undefined && item.lineDiscount > 0;
              const unitPrice = item.customPrice !== undefined ? item.customPrice : item.product.price;
              const rowTotal = unitPrice * item.quantity * (1 - (item.lineDiscount || 0) / 100);
                ...
                >
                  ...
                  <div className="pr-6">
                    ...
                    <div className="flex items-center justify-between text-[11px] text-slate-500 mt-1.5 font-mono">
                      <span>
                        {item.quantity} {item.quantity % 1 === 0 ? 'uds' : 'kg'} x S/ {unitPrice.toFixed(2)}
                        {hasLineDiscount && (
            ...
            {cart.length === 0 && (
              <div className="py-20 text-center text-slate-400 flex flex-col items-center justify-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-500">Ticket Vacío - Mitchell Admin</p>
                  <p className="text-[10px] text-slate-400 mt-1 max-w-[200px] mx-auto">Toca artículos a la derecha para iniciar la simulación de cobro.</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-4 bg-white border-t border-slate-250 space-y-2">
            ...
            <div className="flex justify-between items-center text-[11px] bg-slate-50 border border-slate-200 px-2.5 py-1.5 rounded-lg mb-2">
              <span className="flex items-center gap-1.5 text-slate-600 font-medium">
                ...
                Cliente: <strong className="text-[#714B67] truncate max-w-[150px]">{billingName}</strong>
              </span>
              <button 
            ...
            <div className="space-y-1 font-sans text-xs">
              <div className="flex justify-between text-slate-500 font-semibold">
                ...
                <span className="font-mono">S/ {subtotalBeforeDiscounts.toFixed(2)}</span>
              </div>
              
              {totalLineDiscounts > 0 && (
                <div className="flex justify-between text-purple-600 font-bold">
                  <span>Descuentos de Línea:</span>
                  <span className="font-mono">- S/ {totalLineDiscounts.toFixed(2)}</span>
                </div>
              )}
              ...
              {appliedDiscountRate > 0 && (
                <div className="flex justify-between text-odoo-purple font-bold">
                  ...
                  <span className="font-mono">- S/ {campaignDiscountAmount.toFixed(2)}</span>
                </div>
              )}
              ...
              <div className="pt-2 border-t border-slate-200 flex justify-between items-center bg-[#fcfcff]">
                <span className="text-slate-800 font-extrabold text-xs uppercase tracking-wide">A COBRAR</span>
          ...
          <div className="grid grid-cols-12 bg-[#eeeef2] p-2 border-t border-slate-300 gap-1.5 flex-shrink-0">
            ...
            <div className="col-span-5 flex flex-col gap-1.5 h-full justify-between">
              ...
              >
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] text-teal-200">Sincronizado</span>
                </div>
                <span className="text-[13px] font-black tracking-wide">PAGAR / COBRAR</span>
                <span className="text-[8px] opacity-80 uppercase font-bold">Yape, Plin o Tarjeta</span>
              </button>
            ...
            <div className="col-span-7 grid grid-cols-4 gap-1">
              ...
              <button onClick={() => handleNumpadPress('1')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">1</button>
              <button onClick={() => handleNumpadPress('2')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">2</button>
              <button onClick={() => handleNumpadPress('3')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">3</button>
              <button
              ...
              <button onClick={() => handleNumpadPress('4')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">4</button>
              <button onClick={() => handleNumpadPress('5')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">5</button>
              <button onClick={() => handleNumpadPress('6')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">6</button>
              <button
              ...
              <button onClick={() => handleNumpadPress('7')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">7</button>
              <button onClick={() => handleNumpadPress('8')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">8</button>
              <button onClick={() => handleNumpadPress('9')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">9</button>
              <button
              ...
              <button onClick={() => handleNumpadPress('+/-')} className="h-10 text-xs font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">+/-</button>
              <button onClick={() => handleNumpadPress('0')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">0</button>
              <button onClick={() => handleNumpadPress('.')} className="h-10 text-sm font-bold bg-white active:bg-slate-200 border border-slate-300 rounded-lg shadow-xs flex items-center justify-center cursor-pointer text-slate-800">.</button>
              <button 
        ...
        <div className="lg:col-span-7 flex flex-col bg-[#f0f2f5] overflow-hidden p-4 gap-3 h-[520px] lg:h-full">
          ...
          >
            ...
            <div className="flex-1 min-w-0 pr-1 select-none">
              ...
              <p className="text-[9.5px] leading-snug font-sans text-slate-300">
                Únete a los emprendedores peruanos que ya emiten boletas SUNAT homologadas con total fluidez, rapidez y sin errores de cuadre.
              </p>
            </div>
          </motion.div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              {filteredProducts.length === 0 && (
                <div className="col-span-full py-16 flex flex-col items-center justify-center text-center text-slate-400 bg-white border border-slate-250 rounded-xl gap-2 shadow-xs">
                  ...
                  <p className="text-xs font-bold text-slate-500">Ningún producto coincide</p>
                  <button 
          ...
          <div className="absolute inset-0 bg-slate-950/65 backdrop-blur-xs flex items-center justify-center z-40 p-4">
            ...
            >
              ...
              <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
                ...
                <div>
                  <h4 className="font-extrabold text-xs text-slate-900 tracking-wide uppercase">Asignar Cliente Odoo</h4>
                  <p className="text-[10px] text-slate-400">Emisión de boletas/facturas electrónicas en caja</p>
                </div>
              </div>
              ...
              <div className="space-y-3.5">
                ...
                {/* Billing Name */}
                <div>
                  <label className="block text-[9px] uppercase tracking-wider text-slate-500 font-bold mb-1">Nombre / Razón Social Cliente:</label>
                  <input 
                    type="text" 
                    value={billingName}
                ...
                {/* DNI RUC */}
                <div>
                  <label className="block text-[9px] uppercase tracking-wider text-slate-500 font-bold mb-1">DNI o RUC Tributario:</label>
                  <input 
                    type="text" 
                    value={rucOrDni}
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="block text-[9px] uppercase tracking-wider text-slate-500 font-bold">Código de Descuento (Campaña):</label>
                    <span className="text-[9px] text-[#017e84] font-black">Código: GAOR30</span>
                  </div>
                ...
                <div className="bg-slate-50 p-2 rounded-lg border border-slate-200 text-[10px] text-slate-500 space-y-1">
                  ...
                  <p className="leading-tight text-[9px]">Haz clic en el botón de abajo para cargar automáticamente los datos de simulación del brochure tributario.</p>
            ...
            >
              ...
              <div className="space-y-4 font-sans">
                ...
                <div className="text-center pb-2 border-b border-slate-100">
                  <h4 className="font-extrabold text-[13px] text-slate-900 uppercase tracking-wide">Caja • Procesar Pago</h4>
                  <p className="text-[9px] text-slate-400 uppercase font-mono tracking-wider font-bold mt-0.5">Sincronizado con SUNAT</p>
                </div>

                {/* Amount to pay display */}
                <div className="bg-slate-55 border border-slate-200 rounded-xl p-3 text-center space-y-0.5">
                  <span className="text-[9px] uppercase tracking-wider text-slate-500 font-extrabold block">Total a Cobrar</span>
                  <span className="text-2xl font-black text-[#017e84] font-mono block">S/ {total.toFixed(2)}</span>
                  <span className="text-[9px] text-slate-400 block">(IGV de 18% incluido)</span>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      <span className="bg-emerald-500 text-white text-[7.5px] font-black tracking-wider px-1 py-0.2 rounded uppercase self-start">Caja</span>
                      <span className="text-[11px] font-extrabold tracking-wide uppercase mt-1">💵 Efectivo</span>
                    </button>
                    ...
                    >
                      <span className="bg-purple-600 text-white text-[7.5px] font-black tracking-wider px-1 py-0.2 rounded uppercase self-start">Billetera</span>
                      <span className="text-[11px] font-extrabold tracking-wide uppercase mt-1">📱 Yape</span>
                    </button>
                    ...
                    >
                      <span className="bg-cyan-500 text-white text-[7.5px] font-black tracking-wider px-1 py-0.2 rounded uppercase self-start">Billetera</span>
                      <span className="text-[11px] font-extrabold tracking-wide uppercase mt-1">🟢 Plin</span>
                    </button>
                    ...
                    >
                      <span className="bg-blue-700 text-white text-[7.5px] font-black tracking-wider px-1 py-0.2 rounded uppercase self-start">Tarjeta</span>
                      <span className="text-[11px] font-extrabold tracking-wide uppercase mt-1">💳 Visa / POS</span>
                    </button>
                  </div>
                </div>
                ...
                {selectedPaymentMethod === 'EFECTIVO' ? (
                  <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-2.5 space-y-1.5">
                    ...
                    <div className="flex gap-1.5">
                      <div className="relative flex-1">
                        <span className="absolute left-2 top-1.5 text-xs font-bold text-slate-400">S/</span>
                        <input
                          type="number"
                          value={cashReceived}
                    ...
                    {parseFloat(cashReceived) >= total && (
                      <div className="flex justify-between items-center text-xs font-bold pt-1.5 text-emerald-700 border-t border-emerald-500/10">
                        ...
                        <span className="text-sm font-black font-mono">S/ {(parseFloat(cashReceived) - total).toFixed(2)}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-purple-500/5 border border-purple-500/10 rounded-xl p-2.5 text-center text-[10px] text-slate-500 leading-normal">
                    <p className="font-semibold text-purple-800">📱 Billetera Móvil QR de {selectedPaymentMethod}</p>
                    <p className="mt-0.5">Muestre su código QR al cliente y valide el abono recibido en su app de cobros antes de emitir.</p>
                  </div>
                ) : (
                  <div className="bg-blue-500/5 border border-blue-500/10 rounded-xl p-2.5 text-center text-[10px] text-slate-500 leading-normal">
                    <p className="font-semibold text-blue-800">💳 Cobro mediante Niubiz o Izipay</p>
                    <p className="mt-0.5">Inserte o acerque la tarjeta física del cliente en su terminal POS e ingrese el total de S/ {total.toFixed(2)}.</p>
                  </div>
                )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
            ...
            >
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <h4 className="font-extrabold text-[11px] text-slate-950 font-sans tracking-wide uppercase">Caja Cerrada Exitosamente</h4>
                <p className="text-[8px] text-slate-500 font-bold uppercase tracking-wider mb-1">Boleta autorizada electronicamente</p>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-2.5 my-2.5 font-mono text-[9px] space-y-2.5 shadow-inner leading-normal">
                <div className="text-center space-y-0.5 pb-2 border-b border-dashed border-slate-300">
                  <div className="font-black text-[10.5px] tracking-tight uppercase">GAORSYSTEM COMPROBANTES SAC</div>
                  <div className="text-[8px] text-slate-500 font-bold">RUC: 20601423891 - Lima, Perú</div>
                  <div className="text-[8px] text-slate-500 leading-none">Urb. Las Orquídeas block C8, San Borja</div>
                  <div className="text-[8px] text-slate-500">Soporte: 989 666 214</div>
                </div>

                <div className="space-y-0.5 text-slate-600">
                  <div className="flex justify-between">
                    <span>COMPROBANTE:</span>
                    <span className="font-extrabold text-slate-900">{receiptNumber}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>FECHA:</span>
                    <span>{new Date().toLocaleDateString('es-PE')} {new Date().toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  <div className="flex justify-between max-w-full">
                    <span>CLIENTE:</span>
                    <span className="font-bold text-slate-900 truncate max-w-[130px] text-right">{billingName}</span>
                  </div>
                  {rucOrDni && (
                    <div className="flex justify-between">
                      <span>DNI/RUC:</span>
                ...
                <div className="border-t border-dashed border-slate-300 pt-1.5 space-y-1.5 max-h-[120px] overflow-y-auto pr-0.5">
                  {cart.map((item, idx) => {
                    ...
                    const lineTotal = unitPrice * item.quantity * (1 - discountRate / 100);

                    return (
                      <div key={item.product.id + '-rep-' + idx} className="flex justify-between items-start text-[9px] leading-tight">
                        <div className="max-w-[70%]">
                          <span className="block font-bold text-slate-800 leading-tight">{item.product.name}</span>
                ...
                <div className="border-t border-dashed border-slate-300 pt-1.5 space-y-0.5">
                  ...
                  {selectedPaymentMethod === 'EFECTIVO' && cashReceived && parseFloat(cashReceived) > total && (
                  ...
                  {selectedPaymentMethod === 'EFECTIVO' && cashReceived && parseFloat(cashReceived) > total && (
                    <div className="flex justify-between text-[8.5px] font-black text-emerald-700">
                      ...
                      <span>S/ {(parseFloat(cashReceived) - total).toFixed(2)}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="text-center pt-2.5 border-t border-dashed border-slate-300 text-[9px] text-slate-500 space-y-1.5">
                      ...
                      {Array.from({ length: 49 }).map((_, i) => (
                        <div 
                          ...
                          className={`w-[6px] h-[6px] rounded-xs ${
                            (i * i + 3) % 5 === 0 || i % 7 === 0 || i > 40 ? 'bg-slate-900' : 'bg-slate-100'
                          }`}
                        />
                      ))}
                    </div>
                  ...
                  <div className="font-extrabold text-slate-800">Autorizado mediante resolución SUNAT.</div>
                  <div className="text-odoo-purple font-black text-[9px] tracking-wider uppercase">Fácil • Cloud • Sincronizado</div>
                </div>
              </div>
